...
-- 처음 B를 찾은 위치 조회
SELECT
	'AABBCCABC', INSTR('AABBCCABC', 'B')
FROM
	DUAL; -- 3

-- 5번째 문자 검색 시작, 처음 B를 찾은 위치 조회
SELECT
	'AABBCCABC', INSTR('AABBCCABC', 'B', 5)
FROM
	DUAL; -- 8

-- 5번째 문자 검색 시작, 3번째로 찾은 C의 위치 조회
SELECT
	'AABBCCABC', INSTR('AABBCCABC', 'C', 5, 3)
FROM
	DUAL; -- 9
...
/* (참고)
 * - 문자열 정렬 기준은 글자수, 글자 순서 등에
 * 	 영향이 있기 때문에 정렬 시 생각을 잘 해봐야 함
 * 
...
SELECT
	TO_CHAR(SYSDATE, 'YYYY.MM.DD (DY) PM HH:MI:SS') AS "오늘 날짜"
FROM DUAL;
...
SELECT
	TO_CHAR(SYSDATE, 'RRRR"년" MM"월" DD"일" (DY) PM HH"시" MI"분" SS"초" ')
FROM DUAL;
...
/* 방법 2 - COUNT(컬럼명) 이용 */
SELECT 
	COUNT(PHONE) -- PHONE 컬럼 값이 NULL인 경우를 제외하고 카운트
FROM 
	EMPLOYEE;
