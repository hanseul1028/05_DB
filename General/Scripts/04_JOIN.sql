...
--> 21행 조회
-- 왜? 23행이 아니라 21행?
-- EMPLOYEE에 DEPT_CODE 값이 NULL인 행이 2개 있지만
...
FROM 
	EMPLOYEE EMP
LEFT JOIN
	EMPLOYEE MGR ON (EMP.MANAGER_ID = MGR.EMP_NAME)
ORDER BY 
	EMP.EMP_ID ASC;
...
SELECT DEPT_ID, DEPT_TITLE FROM DEPARTMENT; -- 부서코드, 부서명
...
SELECT
	...
	SALARY 급여
FROM 
	EMPLOYEE E, DEPARTMENT, LOCATION, JOB J
WHERE
...
-- 2. 이름에 '형'자가 들어가는 직원들의 사번, 사원명, 부서명을 조회하시오. O
SELECT	
	E.EMP_ID 사번,
	E.EMP_NAME 이름,
	D.DEPT_TITLE 부서명
