 ...
 * 2) 컬럼명 IS NOT NULL : 해당 컬럼값이 NULL이 아니면 TURE 반환
 * 											 = 컬럼에 값이 존재하면 TRUE
 * --> (컬럼값의 존재 유무를 비교하는 연산)
 * */

-- EMPLOYEE 테이블에서
-- 부서코드(DEPY_CODE)가 없는 사원의
-- 사번, 이름, 부서 코드 조회
SELECT 
 ...
 * - OR(또는) : 두 연산의 결과가 FALSE일 때만 최종 결과 FALSE 
 *	  -> 두 조건 중 하나라도 만족하는 행을 결과집합 (RESULT SET)에 포함
 * - 우선 순위 : AND > OR
 * */
 ...
 * 	 컬럼의 순서도 같이 지정된다
 * 
 * -> ORDER BY절에서 컬럼 순서 이용 가능 (BUT, 권장 X)
 * */
