...
/* 3번 O
 * 노옹철 사원과 같은 부서, 같은 직급인 사원을 조회하시오.
 * (단, 노옹철 사원은 제외)
 * 사번, 이름, 부서코드, 직급코드, 부서명, 직급명
 * */
SELECT
...
SELECT 
	...
	J.JOB_NAME,
	MAIN.EMP_NO AS "만 나이",
	MAIN.SALARY * (1 + NVL(MAIN.BONUS, 0)) * 12 AS 연봉
FROM 
	EMPLOYEE MAIN
JOIN	
	DEPARTMENT D ON (MAIN.DEPT_CODE = D.DEPT_ID)
...
-- 만나이 계산 방법
SELECT EXTRACT(YEAR FROM (CURRENT)) 
FROM EMPLOYEE ;

SELECT 
CEIL((CURRENT_DATE - TO_DATE(EMP_NO,'YYMMMM') / 365) AS 만나이
FROM EMPLOYEE;
